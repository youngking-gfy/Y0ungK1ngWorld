...
再用 SecureCRT 开 3 个窗口 SSH 上去，一个窗口开 Opendaylight，一个窗口开 Mininet,一个配置流表
...
_下面的命令创建具有 2 个交换机，两个交换机下面个连一个主机，交换机之间再互连起来._
...
**_再提醒一下，流表操作在第三个窗口上进行,前面加 sudo_**

**_如果在 mininet 中可以在命令前加上 sh 运行_**
...
- dl_type=0x0800(MAC 帧上一层为 IP 协议)
- nw_src=10.0.0.1(源 IP 地址为 10.0.0.1)
- nw_dst=10.0.0.3(目的 IP 地址为 10.0.0.3)
- 优先级 priority 设为 27，高于其他流表，故优先执行;
- table id 为 0，即将该流表项下发到 table 0 中.
- 该流表项表示:从主机 10.0.0.1 发往主机 10.0.0.3 的 IP 包将被抛弃.
...
通过 OVS 给 S2 下发流表，使得 H1 与 H2 无法互通.
