...
Windows PowerShell 需要用于管理 .NET 对象的语言.该语言需要为使用 cmdlet 提供一致的环境.该语言需要支持复杂的任务,而不会使简单的任务变得更复杂. 该语言需要与在 .NET 编程中使用的高级语言(如 C#)一致.
...
| Windows 版本                  | 7.0 (LTS) | 7.1（最新版） | 7.2 (LTS-preview) |
| ----------------------------- | --------- | ------------- | ----------------- |
...
列出所安装的修复程序(如 QFE 或 Windows Update 文件)
