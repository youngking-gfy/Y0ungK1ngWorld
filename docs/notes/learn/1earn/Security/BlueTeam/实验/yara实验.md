...
这款工具配备一个短小精悍的命令行搜索引擎，它由纯Ｃ语言编写，优化了执行的效率。该引擎可以跨平台使用，能够运行 Windows,Linux 及Ｍ ac OS X 系统上。同时，这款工具提供 python 扩展，允许通过 python 脚本访问搜索引擎。
...
用 exeinfo pe 获取入口点,需要注意的是，这里是获取到的文件偏移，也就是 offset，并不是我们在 IDA 中看到的 entry_point

![](../../../../assets/img/Security/BlueTeam/实验/yara实验/7.png)

![](../../../../assets/img/Security/BlueTeam/实验/yara实验/8.png)

使用 pe.entry_point 来获取入口点
...
> 在 cs3.14 中是$s=New-Object IO.Memor
