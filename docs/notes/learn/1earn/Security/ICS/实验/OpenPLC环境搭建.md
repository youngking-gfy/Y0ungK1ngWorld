...
openplc 提供配套 plc 下位机编程工具，支持梯形图(LD)，功能块图(FBD)，指令列表(IL)，结构化文本(ST)和顺序功能图(SFC)5 种编程语言，程序编写好后，能够将编译后的程序上传到嵌入式平台 (实 plc) 或 X86 平台(软 plc)，并在平台上执行 plc 程序运算。
