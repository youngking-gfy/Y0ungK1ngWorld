...
1. 6×8 的印刷万用板、1300mah 聚合物电池一块(尺寸是 102540)、微动按钮三个、微动开关一个
2. 0.96 OLED 屏幕一块、ESP8266 串口 wifi 模块、micro usb 充电模块一个
3. 电烙铁、焊锡丝、导线若干、热熔胶枪
...
3. 先进行板子布局，因为电路没有设置短路保护而且充电模块紧贴 ESP8266 串口 wifi 模块，为了保险起见先进行充电模块焊接，与测试。
...
5. 进行最后的总线路焊接，下图附成品，因为是飞线，所以外观上不如 pcb 的美观，强度也不如 pcb 高，但不影响功能使用。注意用热熔胶枪加固电池和充电模块。
