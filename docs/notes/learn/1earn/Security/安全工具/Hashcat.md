...
Hashcat 自称是世界上最快的密码恢复工具。它在 2015 年之前拥有专有代码库，但现在作为免费软件发布。适用于 Linux，OS X 和 Windows 的版本可以使用基于 CPU 或基于 GPU 的变体。支持 hashcat 的散列算法有 Microsoft LM hash，MD4，MD5，SHA 系列，Unix 加密格式，MySQL 和 Cisco PIX 等。
...
- [Hashcat 的使用手册总结](https://xz.aliyun.com/t/4008)
- [hashcat rule 的使用](https://darkless.cn/2019/12/26/hashcat-rule/)
...
如果我们想设置字符集为：abcd123456!@-+，就需要用到自定义字符集,hashcat 支持用户最多定义 4 组字符集
...
破解一个原文 8 个字符的 sha256，已知前 5 个字符。
...
- 抢占式实例一次只能买 1 小时,切记 1 小时
- 实例创建完后需要装对应的 NVIDIA 驱动
- -f 测试的跑分速度很完美,但实际跑的时候速度是达不到的

**优点**

- 相对便宜,实际用起来一小时不到 2 元,感觉像在用 vultr
- 性能强
...
CPU 和内存可以不用考虑,主要是选个好的 GPU,这里可以看到有 NVIDIA T4 和 NVIDIA P4 两种选择
