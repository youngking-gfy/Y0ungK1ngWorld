...
- [PHP 伪协议的妙用](https://mp.weixin.qq.com/s/SqGH_AoYForFvh5W52fXkQ)
...
content 中第一个字符 a 就是我们添加的

这个时候我们查看 1.php 的内容如下：
...
这个方法是在 BlackHat 大会上的 Sam Thomas 分享了 File Operation Induced Unserialization via the “phar://” Stream Wrapper ，该研究员指出该方法在文件系统函数 (file_get_contents 、 unlink 等) 参数可控的情况下，配合 `phar://` 伪协议 ，可以不依赖反序列化函数 unserialize() 直接进行反序列化的操作。
