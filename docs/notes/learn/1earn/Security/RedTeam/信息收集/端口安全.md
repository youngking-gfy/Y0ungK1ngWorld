...
2. 2019-11-14:最近发现火绒会拦截本地对外扫描(包括虚拟机) POC 的流量,等我注意到时真是感觉亏了 1 个亿,同理,其他杀软也可能会有这种情况,所以对于漏扫明明爆出漏洞,EXP 却利用不了的时候请看看是不是自己本地的问题.
...
- [SMTP 相关](../协议安全/Protocol-Exploits.md#smtp)
  - 枚举用户
  - 伪造邮件
...
- 概述
  ...
  WHOIS 是一个查询和响应协议，它被广泛用于查询数据库，以存储互联网资源的注册用户相关的信息，如域名、IP 地址块或自治系统，但也用于更广泛的其他信息。
...
- 注入
  ...
  WHOIS 服务始终需要使用数据库来存储和提取信息。 因此，从用户提供的某些信息查询数据库时，可能会出现 SQLInjection。
...
- [Rsync 未授权访问](../软件服务安全/CS-Exploits.md#rsync)
...
- Linux 环境,一般 SSH 也会有个 oracle 的账号,可以爆破一下
- 信息收集,nmap 提供相应 [脚本](../../安全工具/Nmap.md#数据库) , 爆出账号密码可以用 odat 工具尝试 getshell
- [CVE-2012-1675 爆破 TNS](../软件服务安全/CS-Exploits.md#oracle)
- 爆破弱口令
...
- [nfs 未授权访问](../软件服务安全/CS-Exploits.md#nfs)
...
- [ZooKeeper 未授权访问](../软件服务安全/CS-Exploits.md#zookeeper)
...
- [ADB 调试](../../MobileSec/Android安全.md#ADB调试)
...
- [vnc 渗透](../协议安全/Protocol-Exploits.md#vnc)

一般来说,VNC 认证只需要密码,只有在多用户登录的场景才会需要 "用户名"

关于多用户登录的场景可以参考以下 2 篇文章

- [使用 VNC 实现多用户登录 linux 系统](https://www.ibm.com/developerworks/cn/opensource/os-multiuserloginsvnc/index.html)
...
- 爆破弱口令
- [Redis 未授权访问](../软件服务安全/CS-Exploits.md#redis)
...
- 弱口令,ssrf,反序列化
- [weblogic 渗透](../Web安全/BS-Exploits.md#weblogic)
...
- [memcached 未授权访问](../软件服务安全/CS-Exploits.md#memcached)
...
- [Hadoop 未授权访问](../软件服务安全/CS-Exploits.md#hadoop)
